...
ADIR:=$(HOME)/ArduinoBeta/hardware
CUSTOMINCLUDES:=$(HOME)/Dropbox/ArduinoDueGeyser
...
INCLUDES += -I.
INCLUDES += -I$(CUSTOMINCLUDES)/MUXDict -I$(CUSTOMINCLUDES)/SPITemperature -I$(CUSTOMINCLUDES)/CoolingTower -I$(ADIR)/$(SAM)/libraries/SPI -I$(CUSTOMINCLUDES)/ArdAMP -I$(CUSTOMINCLUDES)/ByteBuffer

#compilation flags common to both c and c++
COMMON_FLAGS:=-g -Os -w -ffunction-sections -fdata-sections -nostdlib --param max-inline-insns-single=500 -mcpu=cortex-m3  -mthumb -lSPI -lMUXDict -lSPITemperature -lPID_v1 -lArdAMP -lByteBuffer
...
MYSRCFILES += $(shell ls $(CUSTOMINCLUDES)/MUXDict/*.cpp 2>/dev/null) $(shell ls $(CUSTOMINCLUDES)/SPITemperature/*.cpp 2>/dev/null) $(shell ls $(CUSTOMINCLUDES)/CoolingTower/*.cpp 2>/dev/null) $(shell ls $(ADIR)/$(SAM)/libraries/SPI/*.cpp 2>/dev/null)  $(shell ls $(CUSTOMINCLUDES)/ArdAMP/*.cpp 2>/dev/null) $(shell ls $(CUSTOMINCLUDES)/ByteBuffer/*.cpp 2>/dev/null)
